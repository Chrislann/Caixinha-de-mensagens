...
"Você não é só a pessoa que eu amo, você é o minha melhor amiga, a minha casa, o meu refúgio. Ao seu lado, sinto que posso ser quem realmente sou.",
...
"O amor não é algo que se encontra, é algo que se constrói. Quando encontrei você, soube que minha jornada tinha acabado, porque você é o meu destino.",
  ...
  function showMessage(event) {
    if (shownMessages.length === messages.length) shownMessages = [];
    ...
    document.getElementById("messageText").innerHTML = `<span>${messages[index]}</span>`;
    ...
    document.getElementById("counter").textContent = `${shownMessages.length} / ${messages.length}`;
  ...
  setInterval(createDepthParticle, 1500);
